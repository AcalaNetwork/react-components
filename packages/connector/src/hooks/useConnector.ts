...
function useCachedNetworkConfigs(
	...
	const [cachedConfigs, updateCacheConfigs] = useState<
		Record<ConnectorNetwork, NetworkConfigs>
	>();
	...
	useEffect(() => {
		...
		Object.entries(cachedNetworks).forEach(([name, config]) => {
			if (config.apiType && config.apiType === "rxjs") {
				initRxApi(name as any as ConnectorNetwork, config, dispatch);
			}

			initPromiseApi(name as any as ConnectorNetwork, config, dispatch);
		});
	}, [cachedNetworks]);
};
